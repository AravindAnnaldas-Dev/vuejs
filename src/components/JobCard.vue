...
const truncatedDescription = computed(() => {
  ...
  if (!showToggleButton.value && description.length > props.descriptionLength) {
    description = description.substring(0, props.descriptionLength) + "...";
  }
...
<template>
  <div
    class="flex flex-col items-stretch justify-start gap-2 p-6 transition duration-300 bg-white border shadow-xl border-stone-200 rounded-2xl hover:shadow-xl"
  >
    <!-- Header -->
    <div
    ...
    >
      ...
      <div class="flex items-center gap-2">
        <span class="text-xs text-purple-600">{{
          job.remote === "hybrid" ? "Hybrid" : job.remote ? "Remote" : "On-site"
        }}</span>
        <span
          :class="[
    ...
    <div class="my-2">
      ...
      <p
        v-if="props.job.description.length > props.descriptionLength"
        class="text-green-500 cursor-pointer hover:underline"
        @click="toggleShowMore"
      >
        {{ showToggleButton ? "Less" : "More" }}
      </p>
    </div>

    <!-- Website URL -->
    <p v-if="job.company.website" class="text-sm text-gray-600">
